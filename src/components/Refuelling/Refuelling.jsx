...
import { getDevicesListTC, getTripsListTC } from '../../redux/refuellingReducer'

const Refuelling = (props) => {

  const [RefStart, handleChangeRefStart] = useState(new moment().hour(0).minute(0));
  const [RefFinish, handleChangeRefFinish] = useState(new moment());
  ...
  return (
    <div className="Refuelling d-flex flex-column">
      ...
      <form onSubmit={handleSubmit} className='FormRefuelParams d-flex flex-row justify-content-start align-items-center'>
...
const RefuellingContainer = connect(mapStateToProps, mapDispatchToProps)(Refuelling);
